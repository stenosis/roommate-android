...
public class FilterHelper {
	...
	/**
	 * Checks if one of the filter for the room is set.
	 * 
	 * @return
	 */
	public static boolean hasActiveFilter(boolean[] filter) {
		
		boolean filterActice = false;
		
		for(int i = 0; i < filter.length && !filterActice; i++) {
			
			if(filter[i] == true) {
				filterActice = filter[i];
			}
		}
