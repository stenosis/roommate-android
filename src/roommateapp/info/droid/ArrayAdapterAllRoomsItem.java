...
/**
 * Array adapter for the entire room selection
 * of the ActivityBuilding.
 */
@SuppressLint("NewApi")
public class ArrayAdapterAllRoomsItem extends ArrayAdapter<Room> {
	 ...
	 */
	public ArrayAdapterAllRoomsItem(Context context, int textViewResourceId,
										ArrayList<Room> rooms, ActivityBuilding<?> activityBuilding) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (row == null) {
			...
			row = inflater.inflate(R.layout.filerow_all_rooms, parent, false);
		}
		
		// Name
		TextView textview = (TextView) row.findViewById(R.id.txt_all_room_name);
		textview.setShadowLayer(1, 0, 1, Color.WHITE);
		textview.setText(currentRoom.getName());
		
		// Clicklistener
		RelativeLayout layout = (RelativeLayout) row.findViewById(R.id.layout_allrooms);
		layout.setOnClickListener(new OnCLickListenerRoom(currentRoom));
		
		// Raum size
		textview = (TextView) row.findViewById(R.id.txtSize);
